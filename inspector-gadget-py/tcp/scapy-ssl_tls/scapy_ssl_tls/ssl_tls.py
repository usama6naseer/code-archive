...
class TLSSocket(object):
    def __init__(self, input_tcp_obj=None, sock=None, client=None, tls_ctx=None):
        if input_tcp_obj is not None:
            self.tcp_obj = input_tcp_obj
        # else:
        #     raise ValueError("tcp_obj cannot be None")
        ...
        self.ACK = -1
        self.MSS = -1
        self.CONF_DIR = '/home/usama/github/inspector-gadget-py/conf/'

        try:
            cr = csv.reader(open(self.CONF_DIR + 'conn-info.csv', 'r'))
            print('*********BBB', cr)
            for row in cr:
                print(row)
                self.DST_IP = row[0]
                self.SRC_PORT = int(row[1])
                self.DST_PORT = int(row[2])
                self.EMULATED_RTT = float(row[3])
                self.MSS = int(row[4])
                self.SEQ = int(row[5])
                self.ACK = int(row[6])

        except Exception, e:
            print(e, 'conn-info.csv dont exist')
            sys.exit()

        print('conn-info:', self.DST_IP, self.SRC_PORT, self.DST_PORT, self.EMULATED_RTT, self.MSS)
        print('SEQ/ACK:', self.SEQ, self.ACK)
    ...
    # Custom function
    def send_new(self, pkt_to_be_sent):
        newpkt = IP(dst=self.DST_IP)/TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=self.ACK, window=65535, flags=0, options=[]) / pkt_to_be_sent
        newpkt[TCP].flags = "PA"
        send(newpkt)        
        self.SEQ += len(pkt_to_be_sent)
    ...
    def send_ack(self):
        ...
        # newpkt = IP(dst=self.tcp_obj.dst_ip)/TCP(sport=self.tcp_obj.src_port, dport=self.tcp_obj.dst_port, window=65535, flags=0, options=[("NOP",None),("EOL",None)])
        # newpkt[TCP].flags = "A"
        # newpkt[TCP].seq = ret[0]
        # newpkt[TCP].ack = ret[1]
    ...
    # Custom function
    def send_acks(self):
        for i in self.PACKETS_TO_ACK:            
            newpkt = IP(dst=self.DST_IP)/TCP(sport=self.SRC_PORT, dport=self.DST_PORT, seq=self.SEQ, ack=i[0], window=65535, flags=0, options=[("NOP",None),("EOL",None)])
            newpkt[TCP].flags = "A"
            send(newpkt)
    ...
    def sendall(self, pkt, timeout=2):
        ...
        self._s.settimeout(self.EMULATED_RTT)
        if self.ctx.must_encrypt:
            self.send_new(str(tls_to_raw(pkt, self.tls_ctx, True, self.compress_hook, self.pre_encrypt_hook, self.encrypt_hook)))
        else:
            self.send_new(str(pkt))

        self.tls_ctx.insert(pkt, self._get_pkt_origin('out'))
        self._s.settimeout(prev_timeout)


    def recvall(self, size=8192, timeout=2):
        resp = []
        prev_timeout = self._s.gettimeout()
        self._s.settimeout(self.EMULATED_RTT)
        ...
        while True:
            try:
                ...
                if temp.haslayer(TCP):
                    tcp_flag = temp.sprintf("%TCP.flags%")
                    if temp[TCP].dport == self.tcp_obj.src_port and (tcp_flag == 'PA' or tcp_flag == 'A'):
                        pkt_offset = 14 + (temp[IP].ihl * 4 + temp[TCP].dataofs * 4)
                        pkt_len = temp[IP].len - temp[IP].ihl * 4 - temp[TCP].dataofs * 4

                        if pkt_len > 0:
                            print('recv pkt len:', len(data[pkt_offset:len(data)]), pkt_offset)

                            resp.append(data[pkt_offset:len(data)])
                            self.PACKETS_TO_ACK.append([self.ACK + pkt_len, self.ACK, temp.time])
                            self.ACK += pkt_len
                            print('recv:', self.ACK, temp.time)

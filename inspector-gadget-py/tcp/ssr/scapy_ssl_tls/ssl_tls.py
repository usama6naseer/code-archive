...
class TLSSocket(object):
    ...
    def __init__(self, input_tcp_obj=None, sock=socket.socket(), client=None, tls_ctx=None):
        if input_tcp_obj is not None:
            self.tcp_obj = input_tcp_obj
        else:
            raise ValueError("tcp obj cannot be None")
    ...
    def sendall(self, pkt, timeout=2):
        ...
        if self.ctx.must_encrypt:
            pkt_to_be_sent = str(tls_to_raw(pkt, self.tls_ctx, True, self.compress_hook, self.pre_encrypt_hook, self.encrypt_hook))
            # self._s.sendall(str(tls_to_raw(pkt, self.tls_ctx, True, self.compress_hook, self.pre_encrypt_hook, self.encrypt_hook)))
        else:
            pkt_to_be_sent = str(pkt)
            # self._s.sendall(str(pkt))

        newpkt = IP(dst=self.tcp_obj.dst_ip)/TCP(sport=self.tcp_obj.src_port, dport=self.tcp_obj.dst_port, window=65535, flags=0, options=[("MSS",1460)])/pkt_to_be_sent
        newpkt[TCP].flags = "PA"
        newpkt[TCP].ack = self.tcp_obj.pkt[TCP].ack
        newpkt[TCP].seq = self.tcp_obj.pkt[TCP].seq
        # self.tcp_obj.pkt[TCP].seq = self.tcp_obj.pkt[TCP].seq + (newpkt[TCP].length - 20)
        # send(newpkt) # no response for ack
        # s.send(bytes(p))
        # self._s.sendall(newpkt)
        self._s.sendto(bytes(newpkt),(self.tcp_obj.dst_ip_str,self.tcp_obj.dst_port))
    ...
    def recvall(self, size=8192, timeout=0.5):
        ...
        prev_timeout = self._s.gettimeout()
        self._s.settimeout(timeout)
        build_lfilter = lambda (r): TCP in r and r[TCP].dport == self.tcp_obj.src_port and r[IP].src == self.tcp_obj.dst_ip
        p = sniff(lfilter=build_lfilter, timeout=0.3)
        seq_tracker = {} # see if any pkt has been sent twice due to timeout
        for i in p:
            # print(len(i[TCP]),i[TCP].seq,i[TCP].ack,i[TCP].seq+len(i[TCP])-20,i[TCP].flags,i[TCP].dataofs)
            # if len(i[TCP]) > 26 and i[TCP].flags != 16:
            if len(i[TCP]) > 32:
                if i[TCP].seq not in seq_tracker:
                    # print(len(i[TCP]),i[TCP].flags)
                    seq_tracker[i[TCP].seq] = 1
                    if i.haslayer(Raw):
                        resp.append(i[Raw].load)
